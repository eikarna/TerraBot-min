class ContactManager {
    ...
    async initialize() {
        try {
            ...
            if (this.terra.socket?.store?.contacts) {
                for (const [jid, contact] of Object.entries(
                    this.terra.socket.store.contacts
                )) {
                    if (
                        contact &&
                ...
                this.logger.debug(
                    `Preloaded ${this.contacts.size} contacts from store`
                )
            }
        ...
        try {
            ...
            if (failedAttempt) {
                ...
                if (
                    ...
                    now - lastTry < this.retryDelay
                ) {
                    this.logger.debug(
                        `Skipping fetch for ${jid} - too many recent failures`
                    )
                    return null
                }
            }
            ...
            if (!this.terra.socket || !this.terra.isConnected) {
                this.logger.debug(
                    `Cannot fetch contact ${jid}: Socket not connected`
                )
                return null
            }
            ...
            this.logger.error(
                `Error fetching contact ${jid}: ${error}`.message ||
                    'Unknown error'
            )
            return null
        }
    ...
    async getProfilePicture(jid) {
        try {
            ...
            this.logger.debug(
                `No profile picture available for ${jid}: ${error.message || 'Unknown error'}`
            )
            return null
        }
    }
    ...
    async getName(jid) {
        try {
            ...
            if (cachedContact) {
                ...
                if (cachedContact.verifiedName)
                    return cachedContact.verifiedName
            }
    ...
    async blockUser(jid) {
        try {
            ...
            this.logger.error(
                `Error blocking user ${jid}: ${error.message || 'Unknown error'}`
            )
            return false
        }
    }
    ...
    async unblockUser(jid) {
        try {
            ...
            this.logger.error(
                `Error unblocking user ${jid}: ${error.message || 'Unknown error'}`
            )
            return false
        }
    }
