...
class EventHandler extends EventEmitter {
    ...
    async loadEvents() {
        try {
            ...
            if (files.length === 0) {
                this.logger.warn(
                    'Events directory is empty. Please add event files.'
                )
            }
            ...
            for (const file of eventFiles) {
                try {
                    ...
                    delete require.cache[
                        require.resolve(path.join(eventsDir, file))
                    ]
                    ...
                    if (!event.name || !event.execute) {
                        this.logger.warn(
                            `Event in ${file} is missing required properties`
                        )
                        continue
                    }
    ...
    async handleEvent(eventName, ...args) {
        try {
            ...
            this.logger.error(
                `Error executing event ${eventName}: ${error.message}`
            )
        }
    }
}
