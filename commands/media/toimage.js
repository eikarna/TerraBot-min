    ...
    execute: async (terra, msg, args, context) => {
        try {
            ...
            if (!isSticker) {
                return terra.reply(
                    msg,
                    'Please reply to a sticker with this command to convert it to an image.'
                )
            }
            ...
            try {
                ...
                terra.logger.error(
                    `Error downloading sticker: ${downloadError.message}`
                )
                return terra.reply(
                    msg,
                    `Error downloading sticker: ${downloadError.message}`
            ...
            const workDir = path.join(
                tempDir,
                `sticker_${Date.now()}_${Math.floor(Math.random() * 1000)}`
            )
            ...
            try {
                ...
                if (isAnimated) {
                    ...
                    await terra.reply(
                        msg,
                        '🎞️ Animated sticker detected, converting to GIF...'
                    )
                    ...
                    try {
                        ...
                        terra.logger.error(
                            `Error converting to GIF: ${gifError.message}`
                        )
                        return terra.reply(
                            msg,
                            "Sorry, couldn't convert this animated sticker to GIF."
                        )
                    }
                } else {
                    ...
                    try {
                        ...
                        terra.logger.error(
                            `Error converting to PNG: ${imageError.message}`
                        )
                        return terra.reply(
                            msg,
                            `Error converting sticker to image: ${imageError.message}`
                        )
                    }
                }
            } finally {
            ...
            return terra.reply(
                msg,
                `Error converting sticker: ${error.message}`
            )
        }
    },
}
