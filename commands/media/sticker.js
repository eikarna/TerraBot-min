...
const { downloadMediaMessage } = require('@fizzxydev/baileys-pro') // Add this import
    ...
    execute: async (terra, msg, args, context) => {
        try {
            ...
            if (
                ...
                return terra.reply(
                    msg,
                    'Please send or reply to an image/video, or reply to a message/mention a user to create a profile pic sticker.'
                )
            }
            ...
            try {
                if (shouldUseProfilePic) {
                    ...
                    try {
                        ...
                        const ppUrl =
                            await terra.contactManager.getProfilePicture(
                                targetJid
                            )
                        ...
                        if (!ppUrl) {
                            return terra.reply(
                                msg,
                                "Could not get profile picture. User may not have a profile picture or it's private."
                            )
                        }
                        ...
                        terra.logger.info(
                            `Downloaded profile picture for ${targetJid}`
                        )
                    } catch (ppError) {
                        terra.logger.error(
            ...
            const outputPathSticker = path.join(
                tempDir,
                `output_${randomName}.webp`
            )
            ...
            if (isVideoMedia) {
                ...
                await terra.reply(
                    msg,
                    '🎬 Converting video to sticker (optimizing for WhatsApp size limits)...'
                )
                ...
                await sharp(outputPathGif, { animated: true })
                    .resize(384, 384, {
                        // Reduced from 512x512 to 384x384
                        fit: 'contain',
                ...
                if (await isFileTooLarge(outputPathSticker)) {
                    terra.logger.info(
                        'First sticker attempt was too large, creating smaller version...'
                    )
            ...
            const isOwner =
                terra.config?.owner?.jid?.includes(senderJid) ||
                terra.isOwner(msg.key.remoteJid)
            ...
            if (
                terra.config?.stickers?.ownerPack &&
                isOwner &&
                args.includes('--owner')
            ) {
            ...
            await terra.messageUtils.sendSticker(
                msg.key.remoteJid,
                stickerWithExif,
                { quoted: msg }
            )
            ...
            return terra.reply(
                msg,
                `Error creating sticker: ${error.message}. This might be because the video is too large or in an unsupported format.`
            )
        }
    },
}
